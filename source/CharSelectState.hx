...
class CharSelectState extends MusicBeatState{
    var charsArray:Array<String> = ['BOYFRIEND', 'PICO-PLAYER'];
    var leBG:FlxSprite;
    var bf:FlxSprite;
    var bfcar:FlxSprite;
    ...
    override function create(){
        ...
        add(bf);
        bfcar = new FlxSprite(450, 300).loadGraphic(Paths.image('characters/Pico_FNF_assetss'));
        bfcar.offset.y += 100;
        bfcar.x -= 4;
        bfcar.frames = Paths.getSparrowAtlas('characters/Pico_FNF_assetss');
        bfcar.animation.addByPrefix('idle', 'Pico Idle Dance', 24, true);
        bfcar.animation.addByPrefix('hey', 'Pico Down Note', 24, true);
        bfcar.animation.addByPrefix('singUP', 'pico Up note', 24, true);
        bfcar.animation.play('idle');
        add(bfcar);   
		selectedText = new FlxText(0, 10, charsArray[0], 24);
		selectedText.alpha = 0.5;
        selectedText.offset.x -= 350;
        ...
        //selectedText.x = charSelect.x -= 100;
        selectedText.offset.y = charSelect.y - 100;
        add(charSelect);
        changeSelection();
        super.create();
    }
