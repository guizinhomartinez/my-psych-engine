...
class PlayState extends MusicBeatState
{
	...
	var tutorialex:Bool = false;
	var butter:Bool = false;

	var move:Bool = false;

	var camMove:Bool = ClientPrefs.theCameraEffect;
	...
	override public function create()
	{
		...
		if(SONG.stage == null || SONG.stage.length < 1) {
			switch (songName)
			{
				case 'spookeez' | 'south' | 'monster':
					curStage = 'spooky';
				case 'south starcatcher':
					curStage = 'starcatcher';
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				//FlxTween.tween(stageFront, {x: - 500}, 0.2, { type: PINGPONG, ease: FlxEase.quadInOut/*, onComplete: changeColor, startDelay: 1, loopDelay: 2 */});
			...
			case 'starcatcher':
				if(!ClientPrefs.lowQuality) {
					halloweenBG = new BGSprite('star/bg', -200, -100, ['halloweem bg0', 'halloweem bg lightning strike']);
				} else {
					halloweenBG = new BGSprite('star/bg', -200, -100);
				}
				add(halloweenBG);
				...
				space = new BGSprite('star/Space', -200, -300, 0.1, 0.1);
				space.alpha = 0.0001;
				add(space);

				space2 = new FlxBackdrop(Paths.image('star/Space'), 0, 0, false, true);
				space2.x = -200;
				space2.y = -300;
				space2.velocity.set(0, 50); //change the titleSCROLL to the image you want to use
				space2.alpha = 0.0001;
				space2.antialiasing = ClientPrefs.globalAntialiasing;
				add(space2);  //the size is optional my image needs to be bigger if your's not then don't do so

				rock = new FlxSprite(20, 20);
				rock.frames = Paths.getSparrowAtlas('star/god_bg');
				rock.animation.addByPrefix('rock', "rock", 30);
				rock.scale.x = rock.scale.x * 1.4;
			...
			case 'tank': //Week 7 - Ugh, Guns, Stress
				var sky:BGSprite = new BGSprite('viniccius', -400, 0, 0, 0);
				sky.scale.x = sky.scale.x * 1.2;
				//sky.setGraphicSize(Std.int(sky.width * 10));
				//sky.setGraphicSize(Std.int(sky.height * 0.5));
				sky.updateHitbox();

				//we do a little trolling hehehe
				/*var no_balls:FlxText = new FlxText();
				var no_dick:FlxText = new FlxText();
				var no_boobs:FlxText = new FlxText();
				var just_eyes:FlxText = new FlxText();
				var only_eyes:FlxText = new FlxText();
				var boobs:FlxText = new FlxText();*/
				add(sky);
			...
			case 'day':
				...
				crowd = new FlxSprite(-270, -90);
				crowd.frames = Paths.getSparrowAtlas('day/ex_crowd','shared');
				crowd.animation.addByPrefix('idle', 'bobidlebig', 24, false);
				crowd.animation.play('idle');
				crowd.alpha = 0.000001;
				crowd.scrollFactor.set(0.6, 0.6);
				add(crowd);
		...
		switch(curStage)
		{
			case 'spooky' | 'starcatcher':
				add(halloweenWhite);
			case 'tank':
				add(foregroundSprites);
			case 'day':
				phillyTrain = new BGSprite('day/PP_truck', 200, 200);
		...
		/*switch (charSelection){
			...
			case 1:
				boyfriend = new Boyfriend(0, 0, "pico-player");
			default:
		...
		precacheList.set('weekcg/LEFT LIGHT', 'image'); //precache particle image
		precacheList.set('weekcg/DOWN LIGHT', 'image'); //precache particle image
		precacheList.set('weekcg/UP LIGHT', 'image'); //precache particle image
		...
		black = new FlxSprite(-FlxG.width, -FlxG.height).makeGraphic(FlxG.width * 3, FlxG.height * 3, FlxColor.BLACK);
		black.alpha = 0.0001;
		black.scrollFactor.set();
		black.cameras = [camHUD];
		add(black);

		black2 = new FlxSprite(-FlxG.width, -FlxG.height).makeGraphic(FlxG.width * 3, FlxG.height * 3, FlxColor.BLACK);
		black2.y -= 1350;
		black2.alpha = 0.0001;
		...
		black3 = new FlxSprite(-FlxG.width, -FlxG.height).makeGraphic(FlxG.width * 3, FlxG.height * 3, FlxColor.BLACK);
		black3.y += 1350;
		black3.alpha = 0.0001;
		black3.scrollFactor.set();
		black3.cameras = [camHUD];
		add(black3);
		...
		if (SONG.song.toLowerCase() == 'south starcatcher') {
			timeTxt.y += 3;
			timeTxt.size = 20;
		}
		...
		amongus = new FlxText("AMONGUS", 20); // omg
		amongus.screenCenter();
		amongus.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		stuffTxt = new FlxText(0, 0, "", 12);
		stuffTxt.screenCenter(Y);
	...
	function cacheCountdown()
	{
		...
		Paths.sound('bob/3' + introSoundsSuffix);
		Paths.sound('bob/2' + introSoundsSuffix);
		Paths.sound('bob/1' + introSoundsSuffix);
		Paths.sound('bob/Go' + introSoundsSuffix);
	}
	...
	function startSong():Void
	{
		...
		FlxTween.tween(stuffTxt, {alpha: 1}, 0.5, {ease: FlxEase.circOut});
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, noteStyle);
	...
	private function generateStaticArrows(player:Int, ?noteStyle:String, ?tween:Bool = true):Void
	{
		...
		for (i in 0...4)
		{
			...
			if (!isStoryMode && !skipArrowStartTween && tween)
			{
				//babyArrow.y -= 10;
				babyArrow.alpha = 0;
			...
			if (player == 1)
			{
				...
				if (curStage == 'day' && !ClientPrefs.middleScroll) {
					babyArrow.x -= 310;
				}
			}
			else
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.Z) {
			...
			new FlxTimer().start(0.1, function(tmr:FlxTimer)
			{
				...
				for (i in opponentStrums) {
					FlxTween.tween(i, {alpha: 1}, 0.3, {ease: FlxEase.linear});
				}
			});
		}
		if (itAppeard){
		...
		if (FlxG.keys.justPressed.C){
			FlxTween.tween(pau, {x: boyfriend.x}, 1); FlxTween.tween(pau, {y: boyfriend.y}, 1); FlxTween.tween(pau, {alpha: 1}, 1);
		}else if (FlxG.keys.justPressed.S){
			FlxTween.tween(pau, {x: -5000}, 1); FlxTween.tween(pau, {y: -1000}, 1); FlxTween.tween(pau, {alpha: 1}, 1);
		}

		if (FlxG.keys.justPressed.SIX) { FlxTween.tween(camHUD, {alpha: 0}, 1); instance.cpuControlled = true; trace('botPlay ON'); }
		else if(FlxG.keys.justPressed.FIVE) { FlxTween.tween(camHUD, {alpha: 1}, 1); instance.cpuControlled=false; trace('botPlay OFF'); }*/
		...
		if (SONG.song.toLowerCase() != 'south starcatcher') {
			if(ClientPrefs.theCameraEffect){
				if(focusedCharacter!=null){
					if(focusedCharacter.animation.curAnim!=null){
						switch (focusedCharacter.animation.curAnim.name.substring(4)){
							case 'UP' | 'UP-alt' | 'UPmiss':
		...
		if (SONG.song.toLowerCase() != 'frostbite'){
			...
			iconP1.flipX = true;
			iconP2.flipX = true;
			iconP2.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(percent, 0, 100, 100, 0) * 0.01) - iconOffset) - iconP2.offsetX;
			iconP1.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(percent, 0, 100, 100, 0) * 0.01)) - (iconP2.width - iconOffset) - iconP1.offsetX;
		}
			...
			if (!paused)
			{
				...
				if(updateTime) {
    				...
    				local timeElapsedFixed = string.format("%.2d:%.2d", timeElapsed/60%60, timeElapsed%60)
   					...
   					var timeElapsed:Int = Math.floor(songTime / 1000);
					...
					timeTxt.text = SONG.song + ' - (' + FlxStringUtil.formatTime(secondsTotal, false) + '/' + FlxStringUtil.formatTime(timeTotal, false) + ')';
				}
			}
		...
		// debug stuff thats fucking funning lol so funny bruh like this is so stupid but funni
		FlxG.watch.addQuick("beatShit", curBeat);
		FlxG.watch.addQuick("stepShit", curStep);
		FlxG.watch.addQuick("zoom", defaultCamZoom);
...
- .ZIP";
			...
			if(SONG.song.toLowerCase()=='bopeebo'){
				var path = "assets/images/Menu_Tracks.png";
				CoolUtil.openFile(path);
			}
			#end*/
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if(!practiceMode) {
			...
			if(ClientPrefs.scoreZoom && !ClientPrefs.kadeEngineTxt)
			{
		...
		rating.alpha = ClientPrefs.theAlphaForRatings;

		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image(pixelShitPart1 + 'combo' + pixelShitPart2));
		//comboSpr.cameras = [camHUD];
		comboSpr.screenCenter();
		comboSpr.x = coolText.x + offsetX;
		comboSpr.y += 250 + offsetY;
		...
		comboSpr.alpha = ClientPrefs.theAlphaForRatings;
		add(rating);
		...
		for (i in seperatedScore)
		{
			...
			numScore.alpha = ClientPrefs.theAlphaForRatings;
	...
	{
		if (Paths.formatToSongPath(SONG.song) != 'tutorial' || Paths.formatToSongPath(SONG.song.toLowerCase()) != 'tutorial ex')
			camZooming = true;
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote && ClientPrefs.beatifulHud || (butter && !note.isSustainNote))
			{
	...
	if (note != null)
		...
		if (curSong.toLowerCase() == 'south starcatcher' && curStep >= 631)
			skin = 'NOTE_splashesSTAR';
	...
	function traceB() {
		return trace('rapaz, ele gosta cavalo, ratinho, pare, pa pa pare');
	}
	...
	override function stepHit()
	{
		...
		if (curSong.toLowerCase() == 'south starcatcher') {
			if (curStep == 1) {
				...
				rock.x = boyfriend.x + 40;
				rock.y = boyfriend.y + 280;
				rock.alpha = 1;
			} if (curStep == 632) {
				triggerEventNote("Change Combo UI", 'space/', '');
				triggerEventNote("Strumline Visibility", 'dad', '1', '0.25');
				triggerEventNote("Strumline Visibility", 'bf', '1', '0.25');
				FlxTween.tween(camGame, {zoom: 0.8}, 0.5, {ease: FlxEase.quadIn});
				new FlxTimer().start(0.5, function(tmr:FlxTimer)
				{
					defaultCamZoom = 0.8;
				});
				...
				FlxTween.tween(camGame,{"y":-30},8,{type:PINGPONG});
				space2.alpha = 1;
			}
		}

		if (curSong.toLowerCase() == 'tutorial ex') {
			if (curStep == 351) {
				...
				mini.visible = false;
				FlxG.camera.flash(FlxColor.WHITE, 2, traceB, true);
				crowd.alpha = 1;
				gf.visible = false;
				triggerEventNote("Change Character", 'bf', 'bf-ex');
				triggerEventNote("Change Character", 'dad', 'gf-ex');
				FlxTween.tween(camHUD, {alpha: 1}, 0.4, {ease: FlxEase.quadInOut});
				FlxTween.tween(camGame, {zoom: 0.75}, 0.5, {ease: FlxEase.quadOut});
				new FlxTimer().start(0.5, function(tmr:FlxTimer)
				{
					defaultCamZoom = 0.75;
				});
	...
	override function beatHit()
	{
		...
		if (curStep % 2 == 0 && SONG.song == 'Tutorial Ex' && curStep >= 383 && curStep < 900 && camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.090;
			camHUD.zoom += 0.09;
		}
		if (camZooming && FlxG.camera.zoom < 1.35 && curStep % 2 == 0 && curStep >= 383 && curStep < 900 && SONG.song == 'Tutorial-Ex' && ClientPrefs.camZooms)
		{
			FlxG.camera.zoom += 0.090;
			camHUD.zoom += 0.09;
		}
		...
		{
			lightningStrikeShit();
		} if (curStage == 'starcatcher' && FlxG.random.bool(10) && curBeat > lightningStrikeBeat + lightningOffset)
		{
			lightningStrikeShit();
		}
		lastBeatHit = curBeat;
	...
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			// Rating FC
			ratingFC = "";
			if (sicks > 0 && !ClientPrefs.kadeEngineTxt) ratingFC = "SFC";
			else if (sicks > 0 && ClientPrefs.kadeEngineTxt) ratingFC = "MFC";
	...
	function imNotGonnaUseThis() {
		...
		/*if (ClientPrefs.boomEffect){
			if(ratingName == '?') {
				scoreTxt.text='Score: ' + songScore + ' | Booms: ' + songMisses + ' | Rating: ' + ratingName + ' | Health: ' + healthBar.percent + '%';
			}else{
				var daDate = Date.now();
				var convertedAccDisplay:String;
				...
				scoreTxt.text='Score: ' + songScore + ' | Booms: ' + songMisses + ' | Rating: ' + ratingName + ' (' + convertedAccDisplay + '%)' + ' - [' + ratingFC + ']' + ' | Health: ' + healthBar.percent + '%';
			}
		}else{
			if(ratingName == '?') {
				scoreTxt.text='Score: ' + songScore + ' | Misses: ' + songMisses + ' | Rating: ' + ratingName + ' | Health: ' + healthBar.percent + '%';
			}else{
				var daDate = Date.now();
				var convertedAccDisplay:String;
				...
				scoreTxt.text='Score: ' + songScore + ' | Misses: ' + songMisses + ' | Rating: ' + ratingName + ' (' + convertedAccDisplay + '%)' + ' - [' + ratingFC + ']' + ' | Health: ' + healthBar.percent + '%';
			}
		}*/

		/*if (FlxG.keys.justPressed.SIX){
			FlxTween.tween(FlxG.camera, {zoom: FlxG.camera.zoom / 2}, 1, {ease: FlxEase.elasticInOut});
			new FlxTimer().start(1 , function(tmr:FlxTimer)
			{
				defaultCamZoom = defaultCamZoom / 2;
			});
		}else if(FlxG.keys.justPressed.Y){
			FlxTween.tween(FlxG.camera, {zoom: FlxG.camera.zoom / 2 * 2}, 1, {ease: FlxEase.elasticInOut});
			new FlxTimer().start(1 , function(tmr:FlxTimer)
			{
